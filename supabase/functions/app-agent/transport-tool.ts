...
// Exported so the plan document can reuse the same shapes instead of
// restating them: a leg change must reach both sides at once.
export const place = z.string().min(1).max(200);
export const minute = z.number().int().min(-43200).max(86400);
...
export const reviewTransportParams = z.object({
...
export const reviewTransport = tool({
  ...
  parameters: reviewTransportParams,
