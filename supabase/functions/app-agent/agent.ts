...
export const assistantOutput = z.object({
  ...
  travelContext: travelContextSchema.nullable().default(null).describe('Carry forward confirmed travel facts for the current journey on every turn, including core planning. New user corrections replace old values. Null only when no travel facts are known. Never store raw GPS here.'),
  ...
  quickReplies: z.array(z.object({
    ...
    action: z.enum(['upload_track', 'skip_track', 'request_location']).nullable().default(null),
  })).max(4).nullable().default(null).describe('适合当前问题的快捷回复；不适用时为空数组'),
});
