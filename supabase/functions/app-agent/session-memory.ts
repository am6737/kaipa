...
export function compactionBoundary(rows: SessionRow[], threshold = 16000, keepTurns = 4) {
...
export async function compactSession(rows: SessionRow[], memory: SessionMemory | undefined, summarize: SummarizeMemory, threshold = 16000) {
  ...
  if (!summary || summary.length > 6000) throw new Error('Invalid session memory');
