...
create policy "agent_threads_own" on agent_threads for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
...
begin
  ...
  update agent_runs set status = 'completed', state = null, final_output = assistant_message, pending_approvals = '[]'::jsonb, updated_at = now() where id = target_run_id;
