...
- 不要默认使用中点（`·`）、项目符号、斜杠等标点连接彼此独立的重量、价值、数量、分类等元数据。
- 优先使用图标与数值组合、间距、标签、独立行列或自然换行表达信息结构。
- 只有当标点属于自然语言、约定俗成的数据格式、法律/品牌文案或其他明确的文本表达时才使用。
- 新页面和重设计页面不得机械复制旧有中点分隔样式；修改到相关旧 UI 时应顺手迁移为结构化布局。
