...
import { adminApi, adminMutation, analyzeRouteFact } from '@/lib/supabase'
...
import { FactHistorySheet } from './fact-history-sheet'
import { FactRevisionDialog } from './fact-revision-dialog'
import { serializeFactFields, type FactCategory, type FactEntry, type FactFormValues, type FactRevision, type RouteOption } from './fact-schema'

// A revision proposal is a suggestion that points at an existing entry; it is
// reviewed as a diff rather than confirmed like a draft.
const isRevision = (entry: FactEntry) => entry.status === 'suggested' && Boolean(entry.target_entry_id)
...
export function RouteFacts() {
  ...
  const revisionsQuery = useQuery({ queryKey: ['admin-route-fact-revisions'], queryFn: async () => (await adminApi<{ data: FactRevision[] }>('routeFactRevisions')).data })
  ...
  const rows = useMemo(() => {
    ...
    return (factsQuery.data ?? []).filter((entry) => {
      ...
      else if (statusFilter === 'revision') { if (!isRevision(entry)) return false }
  ...
  const pendingCount = (factsQuery.data ?? []).filter((entry) => entry.status === 'suggested' && !isRevision(entry)).length
  const revisionCount = (factsQuery.data ?? []).filter(isRevision).length
  const expiredCount = (factsQuery.data ?? []).filter(isExpired).length
  const targetOf = (entry: FactEntry) => (factsQuery.data ?? []).find((item) => item.id === entry.target_entry_id) || null
    ...
    <>
      ...
      <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
        <div>
          ...
          <p className='text-muted-foreground'>人工核实的进出交通、费用、住宿、营地等地面信息；AI 规划行程时优先引用这里的数据。{pendingCount > 0 && <span className='text-amber-600'> 有 {pendingCount} 条待确认草稿。</span>}{revisionCount > 0 && <span className='text-amber-600'> 有 {revisionCount} 条修订待审。</span>}{expiredCount > 0 && <span className='text-destructive'> 有 {expiredCount} 条已过期，请复核。</span>}</p>
        </div>
          ...
          <CardContent className='px-0'>
            ...
            <div className='overflow-hidden rounded-md border'>
              <Table className='min-w-4xl'>
                <TableHeader>
                  <TableRow><TableHead>线路</TableHead><TableHead>类目</TableHead><TableHead>标题与要点</TableHead><TableHead>状态</TableHead><TableHead>确认日期</TableHead><TableHead>最后复核</TableHead><TableHead>复核日期</TableHead><TableHead>操作</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {table.getRowModel().rows.map((row) => {
                    ...
                    return <TableRow key={entry.id} className={expired ? 'bg-red-500/5' : undefined}>
                      ...
                      <TableCell className='whitespace-nowrap'>
                        {isRevision(entry)
                          ? <><Badge variant='secondary'>修订待审</Badge><Badge variant='outline' className='ms-1'>AI 草稿</Badge></>
                          : <>
                      ...
                      <TableCell className='whitespace-nowrap text-muted-foreground'>{formatDate(entry.reviewed_at)}</TableCell>
                      ...
                      <TableCell className='whitespace-nowrap'>
                        {isRevision(entry)
                          // No 确认入库 for a revision: confirming one as a new entry is
                          // how a library grows two contradicting facts.
                          ? <Button size='sm' variant='outline' className='me-1' onClick={() => setReview({ open: true, suggestionId: entry.id })}>审核修订</Button>
                          : entry.status === 'suggested' ? <Button size='sm' variant='outline' className='me-1' onClick={() => openEditor(entry)}>确认入库</Button> : null}
                        {entry.status === 'confirmed' && <Button size='sm' variant='outline' className='me-1' onClick={() => runMutation({ action: 'review-route-fact', id: entry.id })}>标记已复核</Button>}
                        {entry.status !== 'archived' && !isRevision(entry) && <Button size='sm' variant='outline' className='me-1' onClick={() => openEditor(entry)}>编辑</Button>}
                  ...
                  {!factsQuery.isPending && table.getRowModel().rows.length === 0 && <TableRow><TableCell colSpan={8} className='h-24 text-center text-muted-foreground'>没有符合条件的资料条目</TableCell></TableRow>}
                </TableBody>
              </Table>
            </div>
        ...
        {review.open && review.suggestionId && (() => {
          const suggestion = (factsQuery.data ?? []).find((entry) => entry.id === review.suggestionId)
          if (!suggestion) return null
          return <FactRevisionDialog
            key={suggestion.id}
            open
            suggestion={suggestion}
            target={targetOf(suggestion)}
            category={categories.find((item) => item.slug === suggestion.category_slug)}
            onOpenChange={(open) => setReview((current) => ({ ...current, open, suggestionId: open ? current.suggestionId : undefined }))}
            onApply={(acceptedFields) => runMutation({ action: 'apply-route-fact-revision', id: suggestion.id, accepted_fields: JSON.stringify(acceptedFields) })}
        ...
        <FactHistorySheet
          ...
          onOpenChange={(open) => setHistory((current) => ({ ...current, open, entryId: open ? current.entryId : undefined }))}
        />
        <FactFormDialog
