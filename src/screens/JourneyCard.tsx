...
export function SelectedPoiCard({ theme, poi, fullBleed, embedded, onTrackSelectionChange, planEditorOpen: controlledPlanEditorOpen, onPlanEditorOpenChange, selectedPlanDays: controlledSelectedPlanDays, onSelectedPlanDaysChange, externalPlanEditorControls = false, onSelectedJourneyDayChange, onJourneyDaysChange, timelineSelectionMode = false, selectedTimelineItemIds, onSelectedTimelineItemIdsChange, detailScrollY, onRequestDetailScroll, scrollContent = false, scrollContentBottomPadding = 18 }: { theme: Theme; poi: Poi; fullBleed?: boolean; embedded?: boolean; onTrackSelectionChange?: (index: number | null, coord?: [number, number]) => void; planEditorOpen?: boolean; onPlanEditorOpenChange?: (open: boolean) => void; selectedPlanDays?: Set<string>; onSelectedPlanDaysChange?: (days: Set<string>) => void; externalPlanEditorControls?: boolean; onSelectedJourneyDayChange?: (day?: string) => void; onJourneyDaysChange?: (days: string[]) => void; timelineSelectionMode?: boolean; selectedTimelineItemIds?: Set<string>; onSelectedTimelineItemIdsChange?: (ids: Set<string>) => void; detailScrollY?: Animated.Value; onRequestDetailScroll?: (y: number) => void; scrollContent?: boolean; scrollContentBottomPadding?: number }) {
  ...
  }, [onSelectedJourneyDayChange, seg, selectedJourneyDay]);
  React.useEffect(
  ...
  return (
    <View style={scrollContent ? { flex: 1, minHeight: 0 } : undefined}>
      ...
      >
          ...
          <>
            ...
            >
                ...
                <>
                  ...
                  <AppCard theme={theme} radius={radius.feature} style={{ marginTop: space.md, backgroundColor: theme.surface }}>
                  ...
                  <React.Fragment key={set.id}>
                    ...
                    >
                      ...
                      <View style={{ flex: 1, minWidth: 0, marginLeft: space.sm }}>
                        <Text numberOfLines={1} style={[type.cardTitle, { color: theme.text }]}>
                          {set.name}
                        </Text>
                        <Text style={[type.caption, { color: theme.text3, marginTop: space.xxs }]}>
                          {t('journey.checklist.itemCount', {
        ...
        {/* 行程 timeline */}
        {seg === 'plan' ? <JourneyTimelineCard theme={theme} info={poi} readOnly={!isJourney} availableDays={journeyDays} /> : null}
        {selectedJourneyDay ? (
          <View
            onLayout={(event) => {
          ...
          >
            <JourneyTimelineCard
              theme={theme}
              info={poi}
              selectedDay={selectedJourneyDay}
              showDayTabs={false}
              ...
              {allPhotos.map((p) => {
                ...
                return (
                  <Press key={p.id} onPress={() => nav.openPhotoWall({ info: poi, mode: 'mine' })} style={{ width: '31.7%' }}>
                    ...
                    >
                      {displayUri ? <Image source={{ uri: displayUri }} contentFit="cover" style={{ width: '100%', height: '100%' }} /> : <PhotoTile tone={p.tone} seed={poi.id + p.id} radius={radius.card} style={{ width: '100%', height: '100%' }} resWidth={420} />}
                      {p.kind === 'video' ? (
                        <View
                          style={{
                            position: 'absolute',
